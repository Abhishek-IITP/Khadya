...
// server-side (keep it on server)
export async function approvesProduct(productId: string) {
  const supabase = await createClient();
  ...
  if (!Number.isInteger(productIdInt)) {
    return {
      ...
      error: {
        code: "INVALID_ID",
        message: `productId is not an integer: "${productId}"`,
      },
    };
  }

  // 2) Fetch the row first (to confirm it exists and to log current values)
  const { data: productData, error: fetchError } = await supabase
    .from("products")
    .select("id, gender, a_status") // select only helpful fields
    .eq("id", productIdInt)
    .limit(1);

  console.log("FETCH RESULT:", { productIdInt, productData, fetchError });
  ...
  if (!productData || productData.length === 0) {
    return {
      ...
      error: {
        code: "NOT_FOUND",
        message: `No product found with id ${productIdInt}`,
      },
    };
  }

  // 3) Attempt update and return updated row using .select(...).single()
  //    (we update gender as a smoke-test; replace with your real column update)
  const { data: updatedData, error: updateError } = await supabase
    .from("products")
    .update({ a_status: "a" })
    .eq("id", productIdInt)
    .select("*");
  // .single(); // return a single object (not array)

  console.log("UPDATE RESULT:", { updatedData, updateError });

  if (updateError) {
    // If updateError is present, return it
    return { success: false, error: updateError };
  }

  if (!updatedData) {
    // No error but no data — likely blocked by RLS or no rows matched
    return {
      success: false,
      error: {
        code: "NO_UPDATE_ROWS",
        message:
          "Update returned no rows. This usually means either no rows matched the filter or Row Level Security blocked the update.",
      },
    };
  }
...
export async function getTopChannels(page: number = 1, limit: number = 10) {
  ...
  // Get total count for only unapproved products
  const { count: totalCount, error: countError } = await supabase
    .from("products")
    .select("*", { count: "exact", head: true })
    .eq("a_status", "na"); // filter here
  ...
  // Fetch paginated unapproved products
  const { data: products, error } = await supabase
    .from("products")
    .select("*")
    .eq("a_status", "na") // filter here too
  ...
  const formattedProducts =
    products?.map((product) => {
      // Ensure productimagesurl is properly formatted
      let productImages = [];
